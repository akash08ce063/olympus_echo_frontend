...
export function TranscriptView({ 
    ...
    const hasTranscript = transcript?.transcript?.call_transcript && transcript.transcript.call_transcript.length > 0
                    ...
                    {hasTranscript ? (
                        <ScrollArea className="h-[calc(100vh-20rem)]">
                            <div className="flex flex-col gap-4 p-4">
                                {transcript.transcript.call_transcript.map((msg: any, msgIdx: number) => {
                        ...
                        <div className="p-8 text-center text-muted-foreground">
                            ...
                            {selectedRunDetailId && onFetchTranscript && (
                                <Button
                                    variant="ghost"
                                    size="sm"
                                    ...
                                    disabled={isFetching}
                                >
                                    {isFetching ? "Fetching..." : "Try Fetching Transcript"}
                                </Button>
                            )}
                        </div>
                    )}
